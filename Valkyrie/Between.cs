...
namespace Valkyrie
{
    ...
    public sealed class BetweenAttribute(object min, object max, string errorMessage = "")
        : ValidationAttribute(string.IsNullOrEmpty(errorMessage) ? "{0} is not between {1} and {2}" : errorMessage)
    {
        /// <summary>
        ...
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            if (Comparer.Compare(MaxValue!, TempValue!) < 0 || Comparer.Compare(TempValue!, MinValue!) < 0)
            {
                return new ValidationResult(FormatErrorMessage(validationContext?.DisplayName ?? ""));
            }
